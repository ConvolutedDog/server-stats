...
def get_secure_logs():
    ...
    sorted_logs = sorted(
        ...
        if f.startswith('secure') and not f.endswith('.tmp') # and f.endswith('secure')
    )
    ...
    for (user, pid), events in sessions.items():
        ...
        if open_time:
            ...
            end_time = datetime.combine(
                target_date,
                time(23, 59, 59)
            ) if target_date < datetime.now().date() else datetime.now()
...
def write_to_csv(daily_usage, target_date):
    ...
    all_usernames = existing_usernames + [
        user for user in current_usernames 
    ...
    for username in all_usernames:
        if username in daily_usage:
            ...
            row_data[username] = round(
                minutes.total_seconds() / 60 if isinstance(minutes, timedelta) 
                else float(minutes), 
                2
            )
        else:
    ...
    try:
        ...
        temp_path = os.path.join(os.path.dirname(csv_file), f"temp_{os.path.basename(csv_file)}")
